...
project's domain model, run `grill-with-docs` here. It's a host-side,
user-interactive step — it sharpens vocabulary against `CONTEXT.md` and
surfaces unstated assumptions before consultants see anything.
...
If the artifact is a written plan or doc, run `critique` against your own
draft before A3. It catches logical fallacies and structural weaknesses
cheaply with one model so consultants spend attention on harder things.

**A3. Consultant review.** Send each selected CLI the review prompt below
with `{focus}` = assessment focus list and `{artifact}` = the A2
assessment. Run consultants in parallel (separate Bash calls in one
message). Dedupe findings across agents — duplicates become one stronger
...
"Reviewers contributed" attribution, and "Unresolved" (your-vs-agent or
agent-vs-agent) with positions side-by-side. **Stop. Do not begin
implementation before the gate.** If the user wants changes, loop back to
A2 or A3. If the user declines, stop entirely.

## Phase B — Implementation

**B1. Implement** the approved assessment. Apply your normal review pass
before handing the diff to consultants — consultant review is on top of
self-review, not a substitute. Run `code-review` against your own diff
here; it parallelizes review dimensions within one model and catches the
obvious stuff so consultants in B2 focus on cross-model disagreement.
...
list and `{artifact}` = the approved assessment (for intent context).
Dedupe findings as in A3.
...
**B3. Discussion (≤2 rounds per agent).** Same protocol as A4. Apply
accepted findings to the diff before B4.

**B4. Gate — present consensus-reviewed code.** Final diff (user runs
`git diff`), summary of fixes attributed by agent, and "Unresolved" with
positions side-by-side. Stop. The user does final review.
...
A3 focus: missed edge cases / failure modes; faulty assumptions about
codebase, libraries, or external systems; underweighted hidden complexity;
better alternatives rejected without concrete reason; missing risks;
whether the approach actually solves the stated problem.
...
- **Implementation-only**: skip Phase A; Phase B as second-opinion code
  review. Good when the plan is obvious.
- **Assessment-only**: Phase A, then implement without B2/B3. Good when
  implementation is mechanical once the plan is right.
- **Single-agent**: one consultant instead of all detected. Lower latency
  and cost; less feedback diversity.
