...
The interview itself happens in the main session (you, talking to the user).
Subagents only enter for the heavy thinking.

## Step 1 — Run the interview

Ask **one** question at a time. Do not batch. Wait for the user's answer before
moving on.
...
1. **What's the decision unmade?** — What does the world look like if you don't
   decide? Default outcome if no action is taken.
...
1. **What would make you change your mind?** — Kill criteria. If X happens, you
   abandon this path.
1. **Gut call right now?** — Force a tentative answer before the analysis. This
   gets recorded so the user can later check whether the analysis moved them or
   just rationalized the gut call.

If the user gives a thin answer ("I dunno"), push once: "Take a guess — what's
the closest thing you do believe?" Move on if they still won't bite, but flag
the gap in the memo.

If the user volunteers extra detail mid-question, capture it but stay on the
ordered list. Don't let the interview wander.

## Step 2 — Spawn the Structurer

Once the interview is done, summarise the captured answers and pass them to a
subagent for structuring.

```text
Agent({
  description: "Structure decision memo",
  subagent_type: "general-purpose",
  prompt: <see Structurer prompt below>
})
```

**Structurer prompt** must include:
...
- Instruction: "If an answer is missing or thin, write `_(unanswered)_` rather
  than inventing content."
- Instruction: "Return the draft memo as Markdown."

Wait for the Structurer to finish before spawning the Contrarian.

## Step 3 — Spawn the Contrarian

Pass the Structurer's draft to a second subagent, instructed to red-team it.

```text
Agent({
  description: "Red-team decision memo",
  subagent_type: "general-purpose",
  prompt: <see Contrarian prompt below>
})
```

**Contrarian prompt** must include:
...
- Required output sections:
  ...
  - **Second-order effects** — what happens 6–18 months out that the user
    hasn't priced in.
  - **Selection bias** — options the user excluded too quickly, or framed in a
    way that prejudged the answer.
...
## Step 4 — Spawn the Synthesizer

Pass both the Structurer draft and the Contrarian critique to a third subagent
to produce the final memo.

```text
Agent({
  description: "Synthesize decision memo",
  subagent_type: "general-purpose",
  prompt: <see Synthesizer prompt below>
})
```
...
- Instruction: "Produce the final memo in the format specified below. You must
  recommend a single option. 'It depends' is not a recommendation. If you
  genuinely cannot recommend, the recommendation is 'gather more information'
  and you must specify exactly which one piece of information would resolve
  it."
...
Create `docs/decisions/` if it doesn't exist. The slug comes from a kebab-case
version of the decision (e.g., `hire-vp-eng`, `kill-pricing-tier-b`).
...
If the user picks "Lock it in", append a `## Decided` section to the memo with
...
- The three passes are kept separate on purpose — structuring, adversarial
  ...
  Don't collapse them into a single agent unless the user explicitly asks for
  a fast version.
- Don't skip the gut-call question. Without it, the synthesised recommendation
  can't be checked against the user's intuition, which is the whole point of
  separating analysis from instinct.
- The memo is a tool for the user, not a deliverable for someone else.
  Optimise for "the user can re-read this in 6 months and remember why" rather
  than for external polish.
